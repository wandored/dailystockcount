...
from dailystockcount.users.forms import (
    RegistrationFrom,
    LoginForm,
    UpdateAccountForm,
    RequestResetForm,
...
from dailystockcount.users.utils import (
    save_picture,
    send_reset_email,
    send_welcome_email,
)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        hashed_password = bcrypt.generate_password_hash(rw.generate()).decode("utf-8")
        user = User(
        ...
        flash(
            "Account has been created and an email has been sent with instructions to reset password!",
            "success",
        )
        return redirect(url_for("users.register"))
    return render_template(
        "users/register.html", title="Add New User", form=form, user_list=user_list
    )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            return (
                redirect(next_page) if next_page else redirect(url_for("main.report"))
            )
        else:
...
def delete_user(user_id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("users.register"))
    return render_template(
        "users/delete_user.html", title="Add New User", form=form, user=user
    )
